...
struct Node {
    ...
    Node(int val) {
        data = val;
        next = nullptr; // Por defecto, un nodo recién creado no apunta a ningún otro
    }
};
...
Node* crearListaDesdeArray(vector<int> arr) {
    ...
    // El puntero 'current' nos ayudará a recorrer la lista mientras la construimos,
    // siempre apuntando al último nodo agregado para poder enlazar el siguiente.
    Node* current = nodo0;
    ...
    for (int i = 1; i < arr.size(); i++) {
        ...
        // para que ahora 'current' sea el "último nodo" y podamos enlazar el siguiente a él.
        current = nuevoNodo;
    }

    // Devolvemos el puntero 'nodo0', que es el inicio de la lista enlazada construida
    return nodo0;
}
...
int main() {
    ...
    cout << "Creando lista enlazada a partir del array: [12, 31, 76, 43, 87]" << endl;
